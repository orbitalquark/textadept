...
// Interface between Textadept and user interfaces (UI).
// Textadept calls these functions to communicate with the UI.
//
// UIs are expected to implement the `main()` entry point of the program, perform any UI-specific
// initialization required on startup, call `init_textadept()` to initialize Textadept (which
// in turn creates the window, etc.), and then start the main event loop. Prior to exiting
// the main loop, UIs will typically want to call `close_textadept()` to clean up and release
// resources before quitting for good.
 ...
 * The UI is expected to implement this (i.e. via a struct).
 */
typedef void Process;

/** Returns the lower-cased name of the UI. */
const char *get_ui(void);
/** Returns the character set used by the UI's filesystem. */
const char *get_charset(void);

/** Asks the UI to create the Textadept window.
 ...
 *
 * At this time, UIs are expected only to construct the bare essentials of the window. Textadept
 * will call on the UI to fill in more details later, such as setting a title, defining menus
 * in a menubar, adding tabs, setting find & replace pane button and option labels, and setting
 * statusbar text. The exception to this is that the UI must call the given function to create
 * the first Scintilla view when it is ready to accept it.
 *
 * Note that Textadept does not create one view per tab, so UIs are expected to keep the tab
 * bar and Scintilla views separate.
 *
 * @param get_view Function to call when the UI is ready to accept the first Scintilla view.
 *	The UI should be ready to create tab for that view at the very least.
 */
...
/** Asks the UI to create and return a new Scintilla view that calls the given callback function
 * with Scintilla notifications.
...
/** Signals the UI to focus the given Scintilla view. */
void focus_view(SciObject *view);
/** Asks the UI to send a message to the given Scintilla view.
...
/** Asks the UI to split the pane holding the given Scintilla view into two views.
...
/** Asks the UI to unsplit the pane the given Scintilla view is in and keep the view.
 * All views in the other pane should be deleted using the given deleter function.
 * If the given view is not in a split pane, the UI should return `false` and do nothing.
...
/** Asks the UI to delete the given Scintilla view.
 * All the UI needs to do here is call Scintilla's UI-specific delete function. The view has
 * already been removed from any split panes (if any).
 */
...
/** Asks the UI to add a tab to the end of its tab list.
 * The UI is not expected to attach anything to the tab, as Textadept does not create one view
 * per tab.
 */
void add_tab(void);
/** Asks the UI to switch to the tab at the given index. */
...
/** Asks the UI to move one of its buffer tabs. */
void move_tab(int from, int to); // 0-based
/** Asks the UI to remove the tab at the given index.
 * As Textadept does not have one view per tab, the UI is not expected to manage any Scintilla
 * views that might have been associated with that tab. It should simply delete the tab and
 * nothing else.
 */
...
/** Asks the UI to add the given text to the find & replace pane's find history list.
 * The given text could be a duplicate. The UI is expected to handle them as it sees fit.
 */
void add_to_find_history(const char *text);
/** Asks the UI to add the given text to the find & replace pane's replace history list.
 * The given text could be a duplicate. The UI is expected to handle them as it sees fit.
 */
...
/** Asks the UI to toggle the state of the find & replace pane.
 * If it is hidden, the UI should show it and set focus to the find entry. If the pane is visible
 * but unfocused, the UI should refocus it. Otherwise, the UI should hide the pane and refocus
 * the focused view.
 */
void focus_find(void);

/** Asks the UI to toggle the command entry between active and hidden.
...
/** Asks the UI to create and return a menu from the Lua table at the given valid index.
 *
...
/** Asks the UI to display the given popup menu.
 * @param menu The menu produced by `read_menu()` to display.
 * @param userdata Userdata for UI use (e.g. from `show_context_menu()`).
 * @see show_context_menu
 */
void popup_menu(void *menu, void *userdata);
/** Asks the UI to read and set a menubar from the Lua table at the given valid index.
...
/** Asks the UI to return the text currently on its clipboard and store the length of that text
 * in the given integer.
 * The UI must return an allocated string -- Textadept will call free() on it.
 */
char *get_clipboard_text(int *len);

/** Asks the UI to run the given function after the given number of seconds.
 * The UI should continue calling `f(reference)` for as long as it returns `true`.
 */
void add_timeout(double interval, bool (*f)(int *), int *reference);

/** Asks the UI to update the UI by painting views, processing any pending events in the main
 * event queue, etc.
...
/** Asks the UI to show a message dialog using the given options.
 * If the user presses a button, the UI should push onto the Lua stack the index (starting from
 * 1) of the button pushed, and then return 1 (the number of results pushed). Otherwise the UI
 * should push nothing and return 0.
 */
int message_dialog(DialogOptions opts, lua_State *L);

/** Asks the UI to show an input dialog using the given options.
 * If the user provides input, the UI should push onto the Lua stack the string input text and
 * then return 1 (the number of results pushed). If `opts.return_button` is `true`, the UI should
 * also push the index (starting from 1) of the button pushed (or 1 to signal an affirmation)
 * and return 2. Otherwise the UI should push nothing and return 0.
 */
int input_dialog(DialogOptions opts, lua_State *L);

/** Asks the UI to show a file selection dialog using the given options.
 * If the user selected a file(s), the UI should push onto the Lua stack the string filename
 * or table of filenames selected, and then return 1 (the number of results pushed). Otherwise
 * the UI should push nothing and return 0.
 * If `opts.multiple` is `true`, the UI should push and populate table, even if only one file
 * was selected.
...
/** Asks the UI to show a file save dialog using the given options.
 * If the user selected a file, the UI should push onto the Lua stack the string filename
 * selected, and then return 1 (the number of results pushed). Otherwise the UI should push
...
/** Asks the UI to show a progress dialog using the given options.
 * The UI is expected to repeatedly call the given `work()` function for as long as it returns
 * `true`. The `update()` function given to `work()` will be called back with progress made so
 * the UI can update its progress bar.
 * If the work was stopped prior to completion, the UI should push onto the Lua stack the the
 * boolean true and return 1. Otherwise, the UI should push nothing and return 0.
 */
...
/** Asks the UI to show a list dialog using the given options.
 ...
 * If the user selected an item(s), the UI should push onto the Lua stack the integer row
 * index or table of row indices (starting from 1) of the item(s) selected, and then return 1
 * (the number of results pushed). If `opts.return_button` is `true`, the UI should also push
 * the index (starting from 1) of the button pushed (or 1 to signal an affirmation) and return
 * 2. Otherwise the UI should push nothing and return 0.
 * If `opts.multiple` is `true`, the UI should push and populate table, even if only one item
 * was selected.
...
/** Asks the UI to spawn a child process asynchronously and return whether or not it
 * successfully spawned that process.
 * When the process exits, it needs to notify Textadept via `process_exited()`.
 * While the UI is allowed to push values to the given Lua state, it may not pop off any values
 * it did not push.
 * @param proc UI-specific process identifier. This pre-allocated chunk of memory should be
 *	filled in by the UI (i.e. its implementing struct's members). The size of `proc` is
 *	defined by `process_size()`.
 * @param index Lua stack index that contains `proc` in case the UI needs to store or refer to it.
 * @param cmd The command line string of the child process to spawn. The UI is expected to
 *	perform any processing needed to launch the process.
 * @param cwd Optional directory to spawn the process in.
 * @param envi Optional stack index of a Lua table that contains a list of "key=value"
 *	environment strings. The UI is expected to read from this table in order to create
 *	a valid process environment. If this index is not provided, the child should inherit
 *	Textadept's environment.
 * @param monitor_stdout Whether or not the UI should notify Textadept of any process stdout
 *	as it comes in via `process_output()`.
 * @param monitor_stderr Whether or not the UI should notify Textadept of any process stderr
 *	as it comes in via `process_output()`.
...
/** Returns the size of the UI's implemented Process struct. */
size_t process_size(void);

/** Returns whether or not the given process is running. */
bool is_process_running(Process *proc);

/** Asks the UI to wait (blocking) until the given process finishes if it has not already. */
void wait_process(Process *proc);

/** Asks the UI to read from the given process' stdout stream, return the data read, and store
 * the number of bytes read in the given pointer.
 * The UI should return NULL on EOF. If a read error occurs, it should return NULL and store
 * the error message and code in the given pointers.
...
/** Asks the UI to write the given data to the given process. */
void write_process_input(Process *proc, const char *s, size_t len);

/** Asks the UI to close the given process' stdin (i.e. send it an EOF). */
void close_process_input(Process *proc);

/** Asks the UI to kill the given process, optionally with the given signal.
 * Not all operating systems support signals, so the UI should use its discretion.
 */
void kill_process(Process *proc, int signal);

/** Returns the exit status of the given process.
 * Textadept will only call this after the UI has indicated the process exited.
 */
int get_process_exit_status(Process *proc);

/** Allows the UI to cleanup a process about to be garbage-collected by Lua. */
void cleanup_process(Process *proc);

/** Asks the UI to suspend execution of Textadept, if possible. */
void suspend(void);

/** Asks the UI quit the application.
