...
local function run_command_helper(command, dir, env, event, commands, key, macros, update)
	...
	local args = {
		command, dir, emit, emit, function(status)
			...
			ui.statusbar_text = status == 0 and _L['Command succeeded'] or _L['Command stopped or failed']
		end
	}
...
-- LuaFormatter off
M.compile_commands = {actionscript='mxmlc "%f"',ada='gnatmake "%f"',c='gcc -o "%e" "%f"',antlr='antlr4 "%f"',g='antlr3 "%f"',applescript='osacompile "%f" -o "%e.scpt"',asm='nasm "%f"'--[[ && ld "%e.o" -o "%e"']],boo='booc "%f"',caml='ocamlc -o "%e" "%f"',csharp=OS=='windows' and 'csc "%f"' or 'mcs "%f"',coffeescript='coffee -c "%f"',context='context --nonstopmode "%f"',cpp='g++ -o "%e" "%f"',cuda=OS=='windows' and 'nvcc -o "%e.exe" "%f"' or 'nvcc -o "%e" "%f"',dmd='dmd "%f"',dot='dot -Tps "%f" -o "%e.ps"',eiffel='se c "%f"',elixir='elixirc "%f"',erlang='erl -compile "%e"',faust='faust -o "%e.cpp" "%f"',fsharp=OS=='windows' and 'fsc.exe "%f"' or 'mono fsc.exe "%f"',fortran='gfortran -o "%e" "%f"',gap='gac -o "%e" "%f"',go='go build "%f"',groovy='groovyc "%f"',hare='hare build -o "%e" "%f"',haskell=OS=='windows' and 'ghc -o "%e.exe" "%f"' or 'ghc -o "%e" "%f"',inform=function() return 'inform -c "'..buffer.filename:match('^(.+%.inform[/\\])Source')..'"' end,java='javac "%f"',ltx='pdflatex -file-line-error -halt-on-error "%f"',less='lessc --no-color "%f" "%e.css"',lilypond='lilypond "%f"',lisp='clisp -c "%f"',litcoffee='coffee -c "%f"',lua='luac -o "%e.luac" "%f"',moon='moonc "%f"',markdown='markdown "%f" > "%e.html"',myr='mbld -b "%e" "%f"',nemerle='ncc "%f" -out:"%e.exe"',nim='nim c "%f"',nsis='MakeNSIS "%f"',objective_c='gcc -o "%e" "%f"',pascal='fpc "%f"',perl='perl -c "%f"',php='php -l "%f"',pony='ponyc "%f"',prolog='gplc --no-top-level "%f"',python='python -m py_compile "%f"',ruby='ruby -c "%f"',rust='rustc "%f"',sass='sass "%f" "%e.css"',scala='scalac "%f"',sml='mlton "%f"',tex='pdflatex -file-line-error -halt-on-error "%f"',typescript='tsc "%f"',vala='valac "%f"',vb=OS=='windows' and 'vbc "%f"' or 'vbnc "%f"',zig='zig build-exe "%f"'}
-- LuaFormatter on
...
-- LuaFormatter off
M.run_commands = {actionscript=OS=='windows' and 'start "" "%e.swf"' or OS=='macos' and 'open "file://%e.swf"' or 'xdg-open "%e.swf"',ada=OS=='windows' and '"%e"' or '"./%e"',c=OS=='windows' and '"%e"' or '"./%e"',applescript='osascript "%f"',asm='"./%e"',awk='awk -f "%f"',batch='"%f"',boo='booi "%f"',caml='ocamlrun "%e"',csharp=OS=='windows' and '"%e"' or 'mono "%e.exe"',chuck='chuck "%f"',clojure='clj -M "%f"',cmake='cmake -P "%f"',coffeescript='coffee "%f"',context=OS=='windows' and 'start "" "%e.pdf"' or OS=='macos' and 'open "%e.pdf"' or 'xdg-open "%e.pdf"',cpp=OS=='windows' and '"%e"' or '"./%e"',crystal='crystal "%f"',cuda=OS=='windows' and '"%e"' or '"./%e"',dart='dart "%f"',dmd=OS=='windows' and '"%e"' or '"./%e"',eiffel="./a.out",elixir='elixir "%f"',fsharp=OS=='windows' and '"%e"' or 'mono "%e.exe"',fantom='fan "%f"',fennel='fennel "%f"',forth='gforth "%f" -e bye',fortran=OS=='windows' and '"%e"' or '"./%e"',gnuplot='gnuplot "%f"',go='go run "%f"',groovy='groovy "%f"',hare='hare run "%f"',haskell=OS=='windows' and '"%e"' or '"./%e"',html=OS=='windows' and 'start "" "%f"' or OS=='macos' and 'open "file://%f"' or 'xdg-open "%f"',icon='icont "%e" -x',idl='idl -batch "%f"',Io='io "%f"',java='java "%e"',javascript='node "%f"',jq='jq -f "%f"',julia='julia "%f"',ltx=OS=='windows' and 'start "" "%e.pdf"' or OS=='macos' and 'open "%e.pdf"' or 'xdg-open "%e.pdf"',less='lessc --no-color "%f"',lilypond=OS=='windows' and 'start "" "%e.pdf"' or OS=='macos' and 'open "%e.pdf"' or 'xdg-open "%e.pdf"',lisp='clisp "%f"',litcoffee='coffee "%f"',lua='lua -e "io.stdout:setvbuf(\'no\')" "%f"',makefile=OS=='windows' and 'nmake -f "%f"' or 'make -f "%f"',markdown='markdown "%f"',moon='moon "%f"',myr=OS=='windows' and '"%e"' or '"./%e"',nemerle=OS=='windows' and '"%e"' or 'mono "%e.exe"',nim='nim c -r "%f"',objective_c=OS=='windows' and '"%e"' or '"./%e"',pascal=OS=='windows' and '"%e"' or '"./%e"',perl='perl "%f"',php='php "%f"',pike='pike "%f"',pkgbuild='makepkg -p "%f"',pony=OS=='windows' and '"%e"' or '"./%e"',prolog=OS=='windows' and '"%e"' or '"./%e"',pure='pure "%f"',python=function() return buffer:get_line(1):find('^#!.-python3') and 'python3 -u "%f"' or 'python -u "%f"' end,rstats=OS=='windows' and 'Rterm -f "%f"' or 'R -f "%f"',rebol='REBOL "%f"',rexx=OS=='windows' and 'rexx "%f"' or 'regina "%f"',ruby='ruby "%f"',rust=OS=='windows' and '"%e"' or '"./%e"',sass='sass "%f"',scala='scala "%e"',bash='bash "%f"',csh='tcsh "%f"',ksh='ksh "%f"',mksh='mksh "%f"',sh='sh "%f"',zsh='zsh "%f"',rc='rc "%f"',smalltalk='gst "%f"',sml=OS=='windows' and '"%e"' or '"./%e"',snobol4='snobol4 -b "%f"',swift='swift "%f"',tcl='tclsh "%f"',tex=OS=='windows' and 'start "" "%e.pdf"' or OS=='macos' and 'open "%e.pdf"' or 'xdg-open "%e.pdf"',vala=OS=='windows' and '"%e"' or '"./%e"',vb=OS=='windows' and '"%e"' or 'mono "%e.exe"',xs='xs "%f"',zig=OS=='windows' and '"%e"' or '"./%e"'}
-- LuaFormatter on
...
-- LuaFormatter off
M.build_commands = {--[[Ant]]['build.xml']='ant',--[[Dockerfile]]Dockerfile='docker build .',--[[Make]]Makefile='make',GNUmakefile='make',makefile='make',--[[Meson]]['meson.build']='meson compile',--[[Maven]]['pom.xml']='mvn',--[[Ruby]]Rakefile='rake',--[[Swift]]['Package.swift']='swift build'}
-- LuaFormatter on
...
function M.goto_error(location)
	...
	if not filename:find(OS ~= 'windows' and '^/' or '^%a?:?[/\\][/\\]?') then
		for i = line_num, 1, -1 do
			local cwd = buffer:get_line(i):match('^> cd ([^\n]+)')
			if cwd then
				filename = cwd .. (OS ~= 'windows' and '/' or '\\') .. filename
				break
			end
		end
	end
	local sloppy = not filename:find(OS ~= 'windows' and '^/' or '^%a?:?[/\\][/\\]?')
